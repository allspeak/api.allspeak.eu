...
def fineTuningFolderOnlyTrain(inputdata_folder, commands_list, output_net_path, output_net_name, train_data, clean_folder=True):
    ...
    #print(str(Nexe) + "-" + str(ninputdata_len))

    if os.path.isfile(init_net_path) is False:
        print("input file (" + init_net_path + ") not found")
        return 0
...
# API call FROM APP  : train a session
# assumes there are two json train_params files (data/train_params.json & data/ft_train_params.json) 
# from whom read train params
# call the proper train function (newnwt OR ft_initnet) according to modeltype param
def train_net(training_sessionid, user_id, modeltype, commands_ids, str_proc_scheme, clean_folder=True):
    ...
    if modeltype == 274:
        ...
        #fineTuningFolderOnlyTrain(folder_path, commands_ids, init_net_path, output_net_path, output_net_name, clean_folder, hm_epochs, batch_size, ftnet_vars_name)

    print('tuning done')
    os.remove(lockfile_path)

def train_net2(training_sessionid, user_id):