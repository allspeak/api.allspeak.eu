...
def train_net(session_data, session_path, training_sessionid, voicebank_vocabulary_path, user_id, clean_folder=False):
    ...
    try:
        ...
        if nModelType == 274:    # PU
            ...
            trainAdapted(training_data_len, ncommands, data_matrix, label_matrix, model_data, output_net_name, session_path, clean_folder)  

        elif nModelType == 276:  # CA (retrieve the pb file of the common session specified)
            ...
            if init_training_session is None:
                ...
                model_data['init_net_path'] = init_training_session.net_path
            trainReAdapted(training_data_len, ncommands, data_matrix, label_matrix, model_data, output_net_name, session_path, clean_folder)
            
        elif nModelType == 278:  # CRA (same as URA)
            init_training_session = TrainingSession.query.filter_by(session_uid=str(session_data['init_sessionid'])).first()
            if init_training_session is None:
                # TODO : raise errors to user... I user_id
                return
        ...
        training_session.net_path = os.path.join(session_path, 'opt' + output_net_name + '.pb')
...
def trainAdapted(training_data_len, ncommands, train_data_matrix, train_label_matrix, model_data, output_model_name, session_path, clean_folder=True):

    # temp folder to create NET files
    train_output_net_path = os.path.join(session_path, 'data', 'net')  # instance/users_data/APIKEY/train_data/training_sessionid/data/net  

    print("adapting net: " + model_data['init_net_path'] + ", creating : " + train_output_net_path)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        # Running first session
        print("Starting ADAPTATION session of file: " + model_data['init_net_path'])
...
def trainReAdapted(training_data_len, ncommands, train_data_matrix, train_label_matrix, model_data, output_model_name, session_path, clean_folder=True):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        # Running first session
        print("Starting RE-ADAPTATION of session of file: " + model_data['init_net_path'])
...
# ===========================================================================================================================
# ===========================================================================================================================
# TRAIN PRIMITIVES        str_proc_scheme = str(session_data['nProcessingScheme'])  # 252/253/254/255/256/257/258
...
#               output_model_name              name of the net file : (sModelFileName, modeltype, proc_scheme, model_class)
...
#                                                                     "saInputNodeName":"inputs/I","sOutputNodeName":"SMO","fRecognitionThreshold":0.1,
